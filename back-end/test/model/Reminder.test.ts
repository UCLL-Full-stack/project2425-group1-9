...
test('Given valid properties, when creating a Reminder, then it should be created successfully', () => {
  const reminder = new Reminder({ id: 1, reminderTime: new Date(Date.now() + 10000), reminderMessage: "reminderMessage"});
...
test('Given an empty reminder time, when creating a Reminder, then it should throw, an error', () => {
  expect(() => new Reminder({ id: 1, reminderTime: null as any, reminderMessage: "reminderMessage"})).toThrowError('Reminder time is required');
});
